...
/**
 * Class containing the main game logic and the print of UI elements
 *
...
public class Game {
    ...
    /**
     * Makes a game cycle
     * Game cycle - One full rotation of all players through the board to the starting position from where they began
     */
    ...
    /**
     * Resets all traps that players may have activated during the game cycle
     */
    ...
    /**
     * Calculates the парички for players
     */
        ...
        for (Player currentPlayer : players) {
            if (currentPlayer.cycle == gameCycle) {
                ...
                if (currentPlayer.pos > 19) {
                    currentPlayer.pos = 0;
                    currentPlayer.cycle++;
                    System.out.println(currentPlayer.getFullPlayerType() + " се премести на позиция " + currentPlayer.pos);
                    continue;
                }
                System.out.println(currentPlayer.getFullPlayerType() + " се премести на позиция " + currentPlayer.pos);
    ...
    /**
     * Removes any player with negative парички balance
     */
    ...
    /**
     * Checks whether there is a winner in the game
     *
    ...
    /**
     * Checks whether a player has a negative парички balance
     *
     * @return {@code true} if his balance is negative, otherwise {@code false}
     */
    ...
    public void shuffleBoard() {
        ...
        for (int i = 1; i < gameBoard.length; i++) {
            newTilePosition = Application.RNG(1, 19);
            Tile.switchTilePosition(gameBoard, i, newTilePosition);
    ...
    private String[] createArray(int firstPos, int lastPos, boolean reverse) {
        ...
        for (int i = 0; i < newArray.length; i++)
            newArray[i] = (reverse) ? convertNum(lastPos - i) : convertNum(firstPos + i);
