...
public class Chance {
    ...
    private void getChancePositivity(Player player) {
        ...
        int dice2 = Application.RNG(1, 100);
    ...
    private int activeDeBuff(Player player) {
        if (player.trapsID[5] == 0) return Application.RNG(1, 10);
    ...
    /**
     * Reads The Chance that the player gets and applies its effects
     *
     * @param rng    The number determining the chance
     * @param offset Used to skip over the negative chances if equals to 5, If equals 0 then a negative chance is red
     * @param player Current player to apply the chance to
     */
